...
int main(int argc, char **argv)
{
  ...
  sonar_back_pub = n.advertise<sensor_msgs::Range>("sonar_back",10);
  ...
  sensor_msgs::Range range, range_back;
  ...
  while (ros::ok())
  {
      ...
      if (nbytes == 0)
        ...
        range_back.field_of_view = 0.5;
        range_back.range = std::min(dis[2], dis[3]);
